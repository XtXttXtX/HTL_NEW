...
class TLVFC:
    ...
    def run(self, from_module: nn.Module, to_module: nn.Module, *args, **kwargs):
        ...
        # initialize fully-connected 初始化全连接层
    ...
    #作用是从特征集合中移除已经匹配迁移过的模块，以统计还剩下哪些模块没被使用
    ...
    #get_all_features(...) 提取两个模型的所有特征层（例如卷积层输出），用于分析
    ...
    @staticmethod
    #用于计算一组 FC 层的整体均值和方差，并按目标层数进行归一化，确保初始化的方差不会太大或太小
