...
#它继承自 Transfer 抽象基类，实现了一个支持 方差对齐（variance alignment） 的参数迁移策略。支持两种类型的层：
#卷积层（Conv2d）：执行通道变换、方差对齐、
#全连接层/偏置（Linear）：执行大小匹配和数值映射
    ...
    def __init__(self, **kwargs) -> None:
        ...
        if len(kwargs) == 0:
            self.var_transfer_kwargs = {
                "type_pad": 'zero',             #填充方式（zero padding）
                "type_pool": 'avg',             #用于特征尺寸对齐的池化方式
                "choice_method": {              #当特征数量不一致时选择保留与舍弃的策略
                    "keep": "interLeaved",
                    "remove": 'random'
                }
            }
        else:
            self.var_transfer_kwargs = kwargs   #默认值全部存在于字典 self.var_transfer_kwargs中

    #迁移的入口函数
    #根据输入的 tensor 维度（特征张量的 shape）决定迁移方式：
